...
FileSystemProvider.prototype.copy = function(props, cb) {
  var self = this,
    readStream,
    writeStream,
    sourcePath = '',
  ...
  if (!validateName(props.sourceContainer, cb)) return;
  if (!validateName(props.destinationContainer, cb)) return;
  if (!validateName(props.sourceFile, cb)) return;
  if (!validateName(props.destinationFile, cb)) return;

  sourcePath = path.join(this.root, props.sourceContainer, props.sourceFile);
  destinationPath = path.join(this.root, props.destinationContainer, props.destinationFile);
...
FileSystemProvider.prototype._isAuthorized = function() {
    return true;
}
