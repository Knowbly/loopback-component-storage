 ...
 * @param {String} destinationContainer Container name
 * @param {String} sourceFile File name to copy
 * @param {String} destinationFile File new name
 * @callback {Function} cb Callback function
 */
StorageService.prototype.copyFile = function(sourceContainer, destinationContainer, sourceFile, destinationFile, cb) {
  var props = { 
  ...
  if (!this.client._isAuthorized()) {
    //Make a dummy request to get the Authentication
    this.client.getFile('test', 'test.jpg', function(err, fileInstance) {
      return self.client.copy(props, cb);
    })
  } else {
    return this.client.copy(props, cb);  
  }
};
...
StorageService.prototype.copyFile.shared = true;
StorageService.prototype.copyFile.accepts = [
  {arg: 'sourceContainer', type: 'string'},
  {arg: 'destinationContainer', type: 'string'},
  {arg: 'sourceFile', type: 'string'},
  {arg: 'destinationFile', type: 'string'}
];
StorageService.prototype.copyFile.returns = {
  arg: 'copied',
  type: 'boolean', root: true
}
StorageService.prototype.copyFile.http =
{verb: 'post', path: '/copy'}
